          ...
          <div className="h-14 md:h-16 flex items-center justify-between">
            ...
            <nav className="flex items-center gap-2 md:gap-3 relative">
              ...
              <AnimatePresence>
                {showLang && (
                  <motion.div
                    className="absolute right-0 top-12 bg-white/95 backdrop-blur border border-slate-200 rounded-xl shadow-lg p-2 w-44"
                  ...
                  >
                    {languages.map(lang => (
                      <button
                        ...
                        className={`w-full flex items-center gap-2 px-2.5 py-2 rounded-lg text-sm text-left hover:bg-slate-50 ${selected === lang.code ? 'bg-slate-50' : ''}`}
                      >
                        <span>{lang.flag}</span>
                        <span>{lang.name}</span>
                      </button>
